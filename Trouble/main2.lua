...
function library:IsTablet()
    if not self:IsMobile() then return false end
    local viewportSize = workspace.CurrentCamera.ViewportSize
    local aspectRatio = viewportSize.X / viewportSize.Y
    return aspectRatio > 1.2 and aspectRatio < 1.8
end

-- 获取自适应尺寸函数
function library:GetAdaptiveSize()
    local viewportSize = workspace.CurrentCamera.ViewportSize
    
    if self:IsMobile() then
        if self:IsTablet() then
            -- 平板尺寸
            return UDim2.new(0.8, 0, 0.85, 0), UDim2.new(0.1, 0, 0.075, 0)
        else
            -- 手机尺寸
            return UDim2.new(0.95, 0, 0.9, 0), UDim2.new(0.025, 0, 0.05, 0)
        end
    else
        -- 电脑尺寸
        return UDim2.new(0, 500, 0, 350), UDim2.new(0.345, 0, 0.291, 0)
    end
end

-- 获取自适应阴影尺寸函数
function library:GetAdaptiveShadowSize(mainSize)
    if self:IsMobile() then
        return UDim2.new(mainSize.X.Scale, mainSize.X.Offset + 20, mainSize.Y.Scale, mainSize.Y.Offset + 20)
    else
        return UDim2.new(0, 520, 0, 370)
    end
end
...
-- 移动设备专用的获取触摸位置函数
function library:GetTouchXY(GuiObject, input)
    if input.UserInputType == Enum.UserInputType.Touch then
        local Max, May = GuiObject.AbsoluteSize.X, GuiObject.AbsoluteSize.Y
        local Px, Py = math.clamp(input.Position.X - GuiObject.AbsolutePosition.X, 0, Max), math.clamp(input.Position.Y - GuiObject.AbsolutePosition.Y, 0, May)
        return Px/Max, Py/May
    else
        return self:GetXY(GuiObject)
    end
end
...
-- 获取自适应尺寸
local mainSize, mainPosition = library:GetAdaptiveSize()
local shadowSize = library:GetAdaptiveShadowSize(mainSize)
...
mainShadow.Position = UDim2.new(mainPosition.X.Scale, mainPosition.X.Offset - 10, mainPosition.Y.Scale, mainPosition.Y.Offset - 10)
mainShadow.Size = shadowSize
...
topbar.BackgroundColor3 = Theme.Topbar
topbar.BorderSizePixel = 0
topbar.Size = UDim2.new(1, 0, 0, library:IsMobile() and 40 or 34)
topbar.Parent = main
...
windowName.TextSize = library:IsMobile() and 16 or 13
...
windowName.Size = UDim2.new(0, library:IsMobile() and 300 or 418, 0, library:IsMobile() and 40 or 34)
...
closeButton.Position = UDim2.new(1, library:IsMobile() and -35 or -20, 0.5, 0)
closeButton.AnchorPoint = Vector2.new(1, 0.5)
closeButton.Size = UDim2.new(0, library:IsMobile() and 22 or 17, 0, library:IsMobile() and 22 or 17)
...
minimizeButton.Position = UDim2.new(1, library:IsMobile() and -65 or -40, 0.5, 0)
minimizeButton.AnchorPoint = Vector2.new(1, 0.5)
minimizeButton.Size = UDim2.new(0, library:IsMobile() and 22 or 17, 0, library:IsMobile() and 22 or 17)
...
minimizeButton.MouseButton1Click:Connect(function()
    ...
    local minimizedSize = UDim2.new(mainSize.X.Scale, mainSize.X.Offset, 0, library:IsMobile() and 40 or 34)
    TweenService:Create(main, TweenInfo.new(.125, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = Minimized and minimizedSize or mainSize}):Play()
...
-- 计算自适应容器尺寸
local tabContainerWidth = library:IsMobile() and 0.25 or 0.262
local containersWidth = 1 - tabContainerWidth

local tabContainer = Instance.new("Frame")
tabContainer.Name = "TabContainer"
tabContainer.BackgroundColor3 = Theme.TabContainer
tabContainer.BorderSizePixel = 0
tabContainer.Position = UDim2.new(0, 0, 0, library:IsMobile() and 40 or 34)
tabContainer.Size = UDim2.new(tabContainerWidth, 0, 1, library:IsMobile() and -40 or -34)
...
tabContainerScrolling.Size = UDim2.new(1, 0, 1, -6)
...
containers.Position = UDim2.new(tabContainerWidth, 0, 0, library:IsMobile() and 40 or 34)
containers.Selectable = false
containers.Size = UDim2.new(containersWidth, 0, 1, library:IsMobile() and -40 or -34)
...
tab.Size = UDim2.new(1, -10, 0, library:IsMobile() and 35 or 29)
...
tabFrame.Size = UDim2.new(1, 0, 0, library:IsMobile() and 30 or 25)
tabFrame.Parent = tab
...
tabName.TextSize = library:IsMobile() and 14 or 12
...
tabFrame.MouseEnter:Connect(function()
    if SelectedTab == nil or SelectedTab ~= tab then
        ...
        TweenService:Create(tabName, TweenInfo.new(.2, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = UDim2.new(1, library:IsMobile() and 8 or 4, 0, library:IsMobile() and 30 or 25)}):Play()
        TweenService:Create(tabFrame, TweenInfo.new(.2, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = UDim2.new(1, library:IsMobile() and 8 or 4, 0, library:IsMobile() and 30 or 25)}):Play()
    end
end)
...
tabFrame.MouseLeave:Connect(function()
    if SelectedTab == nil or SelectedTab ~= tab then
        ...
        TweenService:Create(tabFrame, TweenInfo.new(.2, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = UDim2.new(1, 0, 0, library:IsMobile() and 30 or 25)}):Play()
        TweenService:Create(tabName, TweenInfo.new(.2, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = UDim2.new(1, 0, 0, library:IsMobile() and 30 or 25)}):Play()
    end
end)
...
section.Size = UDim2.new(1, 0, 0, library:IsMobile() and 40 or 33)
...
sectionFrame.Size = UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)
...
sectionName.TextSize = library:IsMobile() and 14 or 12
sectionName.TextXAlignment = Enum.TextXAlignment.Left
sectionName.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
sectionName.BackgroundTransparency = 1
sectionName.Position = UDim2.new(0.0343, 0, 0, 0)
sectionName.Size = UDim2.new(0.9, 0, 1, 0)
...
sectionIcon.Position = UDim2.new(1, -25, 0.5, 0)
sectionIcon.AnchorPoint = Vector2.new(1, 0.5)
sectionIcon.Rotation = 90
sectionIcon.Selectable = false
sectionIcon.Size = UDim2.new(0, library:IsMobile() and 20 or 17, 0, library:IsMobile() and 20 or 17)
...
containerHolder.Position = UDim2.new(0, 0, 1, 0)
...
local SectionY = library:IsMobile() and 35 or 28
local ContainerY = 0

itemContainer.ChildAdded:Connect(function(v)
    if v.ClassName ~= "UIListLayout" then
        if v.Name ~= "Slider" then
        SectionY = SectionY + (library:IsMobile() and 35 or 28)
        ContainerY = ContainerY + (library:IsMobile() and 35 or 28)
        else
        SectionY = SectionY + (library:IsMobile() and 45 or 36)
        ContainerY = ContainerY + (library:IsMobile() and 45 or 36)
        end
    end
end)
...
function sectiontable:Select()
    ...
    section.Size = OpenedSec and UDim2.new(1, 0, 0, SectionY + 5) or UDim2.new(1, 0, 0, library:IsMobile() and 40 or 33)
    sectionFrame.Size = OpenedSec and UDim2.new(1, 0, 0, SectionY) or UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)
    containerHolder.Position = OpenedSec and UDim2.new(0, 0, 1, 0) or UDim2.new(0, 0, 0, 0)
    containerHolder.Size = OpenedSec and UDim2.new(1, 0, 0, ContainerY) or UDim2.new(1, 0, 0, 0)
    sectionIcon.Rotation = OpenedSec and -90 or 90
    sectionIcon.Position = OpenedSec and UDim2.new(1, -22, 0.5, 0) or UDim2.new(1, -25, 0.5, 0)
    itemContainer.Size = OpenedSec and UDim2.new(1, 0, 0, ContainerY) or UDim2.new(1, 0, 0, 0)
end
...
keybind.Size = UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)
...
keybindFrame.Size = UDim2.new(1, -4, 0, library:IsMobile() and 30 or 24)
...
keybindText.TextSize = library:IsMobile() and 14 or 12
...
keybindOuter.AnchorPoint = Vector2.new(1, 0.5)
keybindOuter.BackgroundColor3 = Theme.ToggleOuter
keybindOuter.Position = UDim2.new(1, -5, 0.5, 0)
keybindOuter.Size = UDim2.new(0, library:IsMobile() and 40 or 30, 0, library:IsMobile() and 18 or 14)
...
keybindOuter.Size = UDim2.new(0, math.max(TextBounds.X + 10, library:IsMobile() and 40 or 30), 0, library:IsMobile() and 18 or 14)
...
slider.Size = UDim2.new(1, 0, 0, library:IsMobile() and 45 or 36)
...
sliderFrame.Size = UDim2.new(1, -4, 0, library:IsMobile() and 40 or 32)
...
sliderText.TextSize = library:IsMobile() and 14 or 12
sliderText.TextXAlignment = Enum.TextXAlignment.Left
sliderText.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
sliderText.BackgroundTransparency = 1
sliderText.Position = UDim2.new(0.0234, 0, 0, 0)
sliderText.Size = UDim2.new(1, -10, 0, library:IsMobile() and 20 or 16)
...
sliderValueText.TextSize = library:IsMobile() and 14 or 12
...
sliderValueText.Size = UDim2.new(1, -10, 0, library:IsMobile() and 20 or 16)
...
sliderOuter.Position = UDim2.new(0, 4, 0, library:IsMobile() and 25 or 22)
sliderOuter.Size = UDim2.new(1, -8, 0, library:IsMobile() and 8 or 5)
...
sliderInner.Size = UDim2.new(DefaultScale, 0, 1, 0)
...
dragIcon.Position = UDim2.new(DefaultScale, -5, 0, library:IsMobile() and -2.5 or -2)
dragIcon.Size = UDim2.new(0, library:IsMobile() and 13 or 9, 0, library:IsMobile() and 13 or 9)
...
local function handleSliderDrag()
    ...
    local function updateSlider(input)
        local Px
        if input.UserInputType == Enum.UserInputType.Touch then
            Px = library:GetTouchXY(sliderOuter, input)
        else
            Px = library:GetXY(sliderOuter)
        end
        
        local Value = math.floor(Info.Minimum + ((Info.Maximum - Info.Minimum) * Px))
        Value = math.clamp(Value, Info.Minimum, Info.Maximum)
        local NewScale = (Value - Info.Minimum) / (Info.Maximum - Info.Minimum)
        
        TweenService:Create(sliderInner, TweenInfo.new(0.09, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(NewScale, 0, 1, 0)}):Play()
        local iconpos = math.clamp(NewScale, 0.00981, 0.99141)
        TweenService:Create(dragIcon, TweenInfo.new(0.09, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Position = UDim2.new(iconpos, -5, 0, library:IsMobile() and -2.5 or -2)}):Play()
...
input.Size = UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)
...
inputFrame.Size = UDim2.new(1, -4, 0, library:IsMobile() and 30 or 24)
...
textBox.TextSize = library:IsMobile() and 14 or 12
...
dropdown.Size = UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)
...
dropdownFrame.Size = UDim2.new(1, -4, 0, library:IsMobile() and 30 or 24)
...
dropdownText.TextSize = library:IsMobile() and 14 or 12
...
dropdownTextButton.Size = UDim2.new(1, 0, 1, 0)
...
dropdownIcon.Size = UDim2.new(0, library:IsMobile() and 20 or 17, 0, library:IsMobile() and 20 or 17)
...
DropdownY = DropdownY + (library:IsMobile() and 35 or 24)

if DropdownOpened then
    TweenService:Create(section, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, section.Size.Y.Offset + (library:IsMobile() and 35 or 24))}):Play()
    TweenService:Create(sectionFrame, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, sectionFrame.Size.Y.Offset + (library:IsMobile() and 35 or 24))}):Play()
    TweenService:Create(itemContainer, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, itemContainer.Size.Y.Offset + (library:IsMobile() and 35 or 24))}):Play()
    TweenService:Create(dropdown, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, dropdown.Size.Y.Offset + (library:IsMobile() and 35 or 24))}):Play()
    TweenService:Create(dropdownFrame, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(1, -4, 0, dropdownFrame.Size.Y.Offset + (library:IsMobile() and 30 or 24))}):Play()
    TweenService:Create(dropdownContainer, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, dropdownContainer.Size.Y.Offset + (library:IsMobile() and 35 or 24))}):Play()
    task.wait(.1)
end
...
dropdownElement.Size = UDim2.new(1, 0, 0, library:IsMobile() and 35 or 24)
...
dropdownElementText.TextSize = library:IsMobile() and 14 or 12
...
dropdownElementText.Size = UDim2.new(1, -10, 1, 0)
...
dropdownElementButton.Size = UDim2.new(1, 0, 1, 0)
...
dropdownElementButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(section, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = DropdownOpened and UDim2.new(1, 0, 0, section.Size.Y.Offset + (DropdownY)) or UDim2.new(1, 0, 0, section.Size.Y.Offset - (DropdownY))}):Play()
    TweenService:Create(sectionFrame, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = DropdownOpened and UDim2.new(1, 0, 0, sectionFrame.Size.Y.Offset + (DropdownY)) or UDim2.new(1, 0, 0, sectionFrame.Size.Y.Offset - (DropdownY))}):Play()
    TweenService:Create(itemContainer, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = DropdownOpened and UDim2.new(1, 0, 0, itemContainer.Size.Y.Offset + (DropdownY)) or UDim2.new(1, 0, 0, itemContainer.Size.Y.Offset - (DropdownY))}):Play()
    TweenService:Create(dropdown, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = DropdownOpened and UDim2.new(1, 0, 0, DropdownY + (library:IsMobile() and 35 or 28)) or UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)}):Play()
    TweenService:Create(dropdownFrame, TweenInfo.new(.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {Size = DropdownOpened and UDim2.new(1, -4, 0, DropdownY + (library:IsMobile() and 30 or 24)) or UDim2.new(1, -4, 0, library:IsMobile() and 30 or 24)}):Play()
    ...
    dropdownIcon.Position = DropdownOpened and UDim2.new(1, -22, 0.5, 0) or UDim2.new(1, -25, 0.5, 0)
...
SectionOpened:GetPropertyChangedSignal("Value"):Connect(function()
    ...
    dropdown.Size = DropdownOpened and UDim2.new(1, 0, 0, DropdownY + (library:IsMobile() and 35 or 28)) or UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)
    dropdownFrame.Size = DropdownOpened and UDim2.new(1, -4, 0, DropdownY + (library:IsMobile() and 30 or 24)) or UDim2.new(1, -4, 0, library:IsMobile() and 30 or 24)
    dropdownContainer.Size = DropdownOpened and UDim2.new(1, 0, 0, DropdownY) or UDim2.new(1, 0, 0, 0)
    section.Size = DropdownOpened and UDim2.new(1, 0, 0, section.Size.Y.Offset + (DropdownY)) or UDim2.new(1, 0, 0, section.Size.Y.Offset - (DropdownY))
    sectionFrame.Size = DropdownOpened and UDim2.new(1, 0, 0, sectionFrame.Size.Y.Offset + (DropdownY)) or UDim2.new(1, 0, 0, sectionFrame.Size.Y.Offset - (DropdownY))
    itemContainer.Size = DropdownOpened and UDim2.new(1, 0, 0, itemContainer.Size.Y.Offset + (DropdownY)) or UDim2.new(1, 0, 0, itemContainer.Size.Y.Offset - (DropdownY))
...
toggle.Size = UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)
...
toggleFrame.Size = UDim2.new(1, -4, 0, library:IsMobile() and 30 or 24)
...
toggleText.TextSize = library:IsMobile() and 14 or 12
...
toggleOuter.Position = UDim2.new(1, -40, 0.5, 0)
toggleOuter.AnchorPoint = Vector2.new(1, 0.5)
toggleOuter.Size = UDim2.new(0, library:IsMobile() and 45 or 30, 0, library:IsMobile() and 20 or 14)
...
toggleInner.Size = UDim2.new(0, library:IsMobile() and 16 or 10, 0, library:IsMobile() and 16 or 10)
...
TweenService:Create(toggleInner, TweenInfo.new(.125, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Position = bool and UDim2.new(0, library:IsMobile() and 27 or 18, 0, 2) or UDim2.new(0, 2, 0, 2)}):Play()
...
button.Size = UDim2.new(1, 0, 0, library:IsMobile() and 35 or 28)
...
buttonFrame.Size = UDim2.new(1, -4, 0, library:IsMobile() and 30 or 24)
...
buttonText.TextSize = library:IsMobile() and 14 or 12
...
buttonTextButton.Size = UDim2.new(1, 0, 1, 0)
    ...
    task.spawn(function()
        for _,v in next, tabContainerScrolling:GetChildren() do
            if v.ClassName == "Frame" and v ~= tab then
                ...
                TweenService:Create(v.TabFrame, TweenInfo.new(.2, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = UDim2.new(1, 0, 0, library:IsMobile() and 30 or 25)}):Play()
                v.TabFrame.UIStroke.Color = Theme.TabUIStroke
                TweenService:Create(v.TabFrame.TabName, TweenInfo.new(.2, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), {Size = UDim2.new(1, 0, 0, library:IsMobile() and 30 or 25)}):Play()
            end
        end
    end)
    Left.Visible = true
